<?php $page = 'patient-details-prescription'; ?>   
    ...
    <div class="page-wrapper">
        ...
        <div class="content">
            ...
            <ul class="nav nav-tabs nav-item-primary border-bottom pb-4 mb-4 d-flex align-items-center gap-2">
                ...
                <li class="nav-item">
                    <a href="{{url('patient-details-prescription')}}" class="nav-link border rounded fw-semibold active">
                        Prescription
                    </a>
                </li>
                <li class="nav-item">
                ...
                <div class="card-body">
                    ...
                    <div class="table-responsive table-nowrap">
                        <table class="table mb-0 border">
                            ...
                            <tbody>
                                @forelse($patient as $prescription)
                                <tr>
                                    <td><a href="javascript:void(0);" data-bs-toggle="modal" data-bs-target="#view_modal">{{ $prescription['type'] }}</a></td>
                                    <td>{{ $prescription['quantity'] }}</td>
                                    <td>{{ $prescription['date'] }}</td>
                                    <td>
                                        <div class="d-flex align-items-center">
                                            <a href="{{url('doctor-details')}}" class="avatar avatar-xs me-2">      
                                                <img src="{{ $prescription['doctor_image'] }}" alt="doctor" class="rounded">
                                            </a>
                                            <div>
                                                <h6 class="fs-14 mb-0 fw-medium"><a href="{{url('doctor-details')}}">{{ $prescription['doctor_name'] }}</a></h6>
                                            </div>
                                        </div>
                                    </td>
                                    <td>{{ $prescription['amount'] }}</td> 
                                    <td>{{ $prescription['payment'] }}</td>                                      
                                    <td>
                                        <span class="badge badge-soft-{{ ['Completed' => 'success', 'Pending' => 'warning'][$prescription['status']] ?? 'purple' }}">
                                        {{ $prescription['status'] }}
                                        </span>
                                    </td>
                                    <td class="text-end">
